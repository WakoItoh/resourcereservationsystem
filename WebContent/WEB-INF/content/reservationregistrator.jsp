...
<c:forEach var="msg" items="${errorMessageList}">
	...
	<form action="reservationregister" method="post">
		<div class="field">
			...
			<div class="normal">
				<table class="time">
					...
					<tbody>
						<tr>
							<c:forEach begin="0" end="95" var="time" items="${timeList}"><td<c:forEach var="availableResource" items="${availableResourceList}"><c:if test="${availableResource.availableStartTime <= time && time < availableResource.availableEndTime}"> class="available"</c:if></c:forEach>></td></c:forEach>
						</tr>
					</tbody>
				</table>
			</div>
		...
		<div class="field">
			...
			<div class="normal">
				<input type="hidden" id="resource" name="resource" value="<c:out value="${resource.resourceId}" />">
				<c:out value="${resource.resourceName}" />
			</div>
		</div>
		<div class="field">
		...
		<div class="field">
			<input class="medium button-submit" type="submit" value="予約"><a class="medium button" href="reservableresource">戻る</a>
		</div>
	</form>
</div>
</body>
